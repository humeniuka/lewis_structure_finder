...
def disconnected_fragments(atomlist, **kwds):
    """
    ...
    """
    fragment_graphs = atomlist2graph(atomlist, **kwds)
    fragments = []
...
def fragment_molecule(atomlist, **kwds):
    """
    divide a molecule into disconnected fragments
    ...
    Returns
    -------
    fragment_indices : list of lists of indices into atomlist
       fragment_indices[ifrag] is the list of atom indices belonging to the
       disconnected fragment `ifrag`
    """
    # number of atoms
    nat = len(atomlist)
    ...
    fragment_indices = [np.sort(np.array(graph2indeces(g), dtype=int)) for g in fragment_graphs]
