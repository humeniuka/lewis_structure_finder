...
from numpy.linalg import eigh, inv, det
import numpy as np
...
def distance(atom1, atom2):
    ...
    (Z1,pos1) = atom1
    (Z2,pos2) = atom2
    d = la.norm(np.array(pos1) - np.array(pos2))
    return d
...
def angle(atom1, atom2, atom3):
    ...
    """
    v12 = np.array(atom2[1]) - np.array(atom1[1])
    v13 = np.array(atom3[1]) - np.array(atom1[1])
    a = np.arccos(np.dot(v12,v13)/(la.norm(v12)*la.norm(v13)))
    return a
...
def dihedral_angle(atom1, atom2, atom3, atom4):
    ...
    """
    b1 = np.array(atom2[1]) - np.array(atom1[1])
    b2 = np.array(atom3[1]) - np.array(atom2[1])
    b3 = np.array(atom4[1]) - np.array(atom3[1])
    n12 = cross(b1,b2)
    n23 = cross(b2,b3)
    dihedral = np.arctan2(np.dot(cross(n12,n23),b2/la.norm(b2)), np.dot(n12,n23))
    return dihedral
...
def molpro_standard_orientation(masses, pos, verbose=0):
    """
